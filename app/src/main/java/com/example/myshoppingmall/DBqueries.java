                ...
                if(task.isSuccessful()){
                    for(long x = 0; x < (long)task.getResult().get("list_size"); x++){
                        ...
                        if(DBqueries.wishList.contains(ProductDetailsActivity.productID)){
                            ProductDetailsActivity.ALREADY_ADD_TO_WISHLIST = true;
                            if(ProductDetailsActivity.addToWishListBtn != null) {
                                ProductDetailsActivity.addToWishListBtn.setSupportImageTintList(context.getResources().getColorStateList(R.color.primaryRed));
                            }
                        } else{
                            if(ProductDetailsActivity.addToWishListBtn != null) {
                                ProductDetailsActivity.addToWishListBtn.setSupportImageTintList(ColorStateList.valueOf(Color.parseColor("#9e9e9e")));
                            }
                            ProductDetailsActivity.ALREADY_ADD_TO_WISHLIST = false;
                        }
                ...
                if(task.isSuccessful()){
                    for(long x = 0; x < (long)task.getResult().get("list_size"); x++){
                        ...
                        if(DBqueries.cartList.contains(ProductDetailsActivity.productID)){
                            ProductDetailsActivity.ALREADY_ADD_TO_CART = true;
                        } else{
                            ProductDetailsActivity.ALREADY_ADD_TO_CART = false;
                        }
